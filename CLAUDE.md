...
src/DocsFlow.Spaces/             space и контексты: членство, роли, единица изоляции данных
...
**Space** — группа доступа и единица изоляции: всё содержимое архива принадлежит ровно одному
space. **Контекст** — тематическое направление внутри space («авто», «ремонт»), плоский список.
Владелец не колонка в `spaces`, а роль в `space_members`: проверка доступа одна — есть ли строка.

Членство проверяет `SpaceMembershipFilter`, повешенный на группу маршрутов `/api/spaces/{spaceId}`.
Эндпоинт получает проверку по факту объявления в группе и читает результат через
`SpaceAccess.Of(http)`. Параметром обработчика этот тип не сделать: минимальные API привязывают
параметры раньше, чем выполняется первый фильтр.

Два правила, которые нельзя нарушать в новом коде:

- **Идентификатор space берётся из маршрута**, а не из тела запроса, и всегда сверяется с членством.
- **Чужой space неотличим от несуществующего** — обоим 404. 403 остаётся только там, где о
  существовании space спрашивающий и так знает: управление составом и именем участником.

Пайплайн RAG принимает `spaceId` обязательным параметром во всех методах — метода, работающего со
всей таблицей, в контракте нет намеренно.
