...
Приложение работает за nginx, поэтому запрос доходит до него по http даже когда браузер разговаривал
с прокси по https. Схема из запроса идёт в собираемые адреса — `redirect_uri` и
`post_logout_redirect_uri`, — так что без разбора `X-Forwarded-Proto` приложение за TLS собрало бы
адрес возврата, которого нет в realm, а `UseHttpsRedirection` начал бы зацикливать редиректы.
`UseForwardedHeaders` стоит **первым** в конвейере: всё, что ниже, должно видеть уже исправленные
схему и адрес клиента.

Разбираются только `X-Forwarded-Proto` и `X-Forwarded-For`. `X-Forwarded-Host` — нет: nginx передаёт
исходный `Host` как есть (`proxy_set_header Host $host`), и доверие второму заголовку с тем же
смыслом дало бы лишний способ подменить адрес.

Доверенные прокси — секция **`Network:TrustedProxies`**: `Addresses` (отдельные адреса) и `Networks`
(CIDR). Список обязателен именно потому, что эти заголовки подделываются кем угодно: доверяя им без
разбора, приложение начинает верить чужому мнению о схеме запроса и адресе клиента — в логах и в
защите от перебора оказывается подставной IP, а `https` в заголовке заставляет считать защищённым
соединение, пришедшее по http. Пустой список означает «только loopback», как и по умолчанию в
ASP.NET; свои значения **добавляются** к дефолтным, а не заменяют их. Неверный адрес или CIDR роняет
запуск: молча проигнорированный прокси означал бы, что приложение тихо перестало доверять
заголовкам. Локально в `appsettings.Development.json` прописана `172.16.0.0/12` — из этой сети
приходит nginx в Docker.
...
Адреса возврата — `/api/auth/signin-callback` и `/api/auth/signout-callback` на трёх origin:
`localhost:8080` (nginx с веб-клиентом — так приложение видит браузер), `localhost:5023` и
`localhost:7207` (API напрямую через `dotnet run`, порты из `launchSettings.json`).

**Колбэки лежат под `/api/`, а не на дефолтных `/signin-oidc` и `/signout-callback-oidc.`** За
nginx на бэкенд проксируется только `/api/`, всё остальное отдаётся как статика клиента, поэтому
колбэк вне этого префикса до приложения не дошёл бы — вместо него вернулся бы `index.html`.
Альтернатива — добавить в nginx отдельные `location` под каждый колбэк; отклонена как более
хрупкая: правило пришлось бы держать в двух местах и синхронизировать при каждом изменении путей.
...
2. Полный вход: `HttpClient` с `CookieContainer` и `AllowAutoRedirect`, `GET /api/auth/login` →
   ...
   `/api/auth/signin-callback` → `GET /api/me` отдаёт `200` с email тестового пользователя. Далее — в таблице
...
- **Два `RedirectContext`.** У cookie-схемы он обобщённый и лежит в
  ...
  алиасом.
- **`redirect_uri` не появляется в ссылке на Keycloak.** Начиная с .NET 9 OIDC-обработчик по
  умолчанию использует pushed authorization request: параметры авторизации уходят в Keycloak
  отдельным backchannel-запросом, а в редиректе остаются только `client_id` и `request_uri`. Два
  следствия. Незарегистрированный адрес возврата отвергается **на входе в `/api/auth/login`** и
  вылезает как `500` с `OpenIdConnectProtocolException`, а не как страница ошибки Keycloak после
  редиректа. И проверять схему собранного адреса приходится по `post_logout_redirect_uri` — он
  через PAR не уходит и в ссылке виден.
...
- Список доверенных прокси для продакшна: сети, из которых приходит настоящий балансировщик.
  Сам разбор заголовков уже включён (см. «Заголовки от прокси»), не заполнен только список.
