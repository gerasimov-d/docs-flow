...
- **`UseForwardedHeaders` в API.** Локально это не проявляется: nginx и приложение работают по
  http, поэтому `redirect_uri`, собранный из запроса, совпадает с зарегистрированным. Как только
  TLS будет терминироваться на прокси, приложение продолжит видеть схему `http` и соберёт адрес
  возврата, которого нет в realm. `X-Forwarded-Proto` nginx уже присылает — не хватает только
  middleware, доверяющего ему, и списка доверенных прокси.
